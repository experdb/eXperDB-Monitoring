...
Public Class frmLoginB
    ...
    Private Sub btnLogin_MouseEnter(sender As Object, e As EventArgs) Handles btnLogin.MouseEnter, btnClose.MouseEnter
        Dim tmpButton = DirectCast(sender, System.Windows.Forms.Button)
        tmpButton.BackColor = System.Drawing.Color.FromArgb(CType(CType(220, Byte), Integer), CType(CType(120, Byte), Integer), CType(CType(196, Byte), Integer), CType(CType(255, Byte), Integer))
    End Sub
...
#Region "Movable Form"
    ...
    Private Sub pnlDesc_MouseDown(sender As Object, e As MouseEventArgs) Handles pnlDesc.MouseDown, lblDesc1.MouseDown, lblDesc2.MouseDown, lblDesc3.MouseDown, _
                                                    lblMain.MouseDown, lblSub.MouseDown
        If (e.Button = MouseButtons.Left) Then
            ReleaseCapture()
            SendMessage(Handle, WM_NCLBUTTONDOWN, HT_CAPTION, 0)
        End If
    ...
    Private Function DoLogin() As Boolean
        Try
            ...
            _cs = New clsSession(txtUserID.Text, "", isAdmin, loginDt)
            '''''''''''''''' check expiration pw  '''''''''''''''''''''''''''''''''''''''''''''''''''''
            If isExpired Then
