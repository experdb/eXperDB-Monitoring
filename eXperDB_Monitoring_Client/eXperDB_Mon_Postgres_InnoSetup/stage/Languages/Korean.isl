...
SetupFileMissing=설치 폴더에 %1 파일이 없습니다. 문제를 해결해 주시거나 새로운 설치 프로그램을 구해보세요.
SetupFileCorrupt=설치 파일이 손상되었습니다. 새로운 설치 프로그램을 구해보세요.
SetupFileCorruptOrWrongVer=설치 파일이 손상되었거나 이 버전의 설치 프로그램과 호환이 되지 않습니다. 문제를 해결해 주시거나 새로운 설치 프로그램을 구해보세요.
...
MissingWOW64APIs=실행 중인 Windows 가 설치 프로그램이 64비트 설치를 수행하기 위한 기능을 포함하고 있지 않습니다. 이 문제를 해결하려면, 서비스 팩 %1 을(를) 설치하세요.
...
SetupAppRunningError=설치 프로그램이 %1 이(가) 실행중인 것을 감지했습니다.%n%n지금 관련 프로그램을 모두 닫으십시오. 설치를 계속 하려면 "확인"을, 취소하려면 "취소"를 클릭하세요.
UninstallAppRunningError=설치 제거 프로그램이 %1 이(가) 실행중인 것을 감지했습니다.%n%n지금 관련 프로그램을 모두 닫으십시오. 설치를 계속 하려면 "확인"을, 취소하려면 "취소"를 클릭하세요.
...
ButtonYesToAll=모두 예
ButtonNo=아니오
ButtonNoToAll=모두 아니오
ButtonFinish=완료
ButtonBrowse=탐색...
ButtonWizardBrowse=탐색...
ButtonNewFolder=폴더 만들기
SelectLanguageTitle=설치 언어 선택
SelectLanguageLabel=설치 과정 중에 사용할 언어를 선택하세요:
ClickNext=설치를 계속 하려면 "다음"을, 종료하려면 "취소"를 클릭하세요.
BeveledLabel=
BrowseDialogTitle=폴더 탐색
BrowseDialogLabel=아래의 목록에서 폴더를 선택하고 "확인"을 클릭하세요.
NewFolderName=새 폴더
WelcomeLabel1=[name] 의 설치 마법사에 오신 것을 환영합니다
WelcomeLabel2=이 프로그램은 [name/ver] 을(를) 설치할 것입니다.%n%n설치를 계속하기 전에 실행 중인 모든 프로그램을 종료할 것을 권장합니다.
WizardPassword=비밀번호
PasswordLabel1=이 프로그램의 설치는 비밀번호로 보호되고 있습니다.
PasswordLabel3=비밀번호를 입력하신 후 "다음"을 클릭해 주세요. 비밀번호는 대소문자를 구분합니다.
PasswordEditLabel=비밀번호:
IncorrectPassword=입력하신 비밀번호는 올바르지 않습니다. 다시 입력하세요.
WizardLicense=사용자 계약
LicenseLabel=설치를 계속하기 전에 아래의 중요한 정보를 꼭 읽어 보세요.
LicenseLabel3=다음 사용자 계약을 자세히 읽어 보세요. 설치를 계속 하려면 이 계약에 동의해야 합니다.
LicenseAccepted=사용자 계약에 동의합니다
LicenseNotAccepted=사용자 계약에 동의하지 않습니다
WizardInfoBefore=정보
InfoBeforeLabel=설치를 계속하기 전에 아래의 중요한 정보를 꼭 읽어 보세요.
InfoBeforeClickLabel=설치를 계속할 준비가 되었다면, "다음"을 클릭해 주세요.
WizardInfoAfter=정보
InfoAfterLabel=설치를 계속하기 전에 아래의 중요한 정보를 꼭 읽어 보세요.
InfoAfterClickLabel=설치를 계속할 준비가 되었다면, "다음"을 클릭하세요.
WizardUserInfo=사용자 정보
UserInfoDesc=사용자 정보를 입력하세요.
UserInfoName=사용자 이름:
UserInfoOrg=조직:
UserInfoSerial=시리얼 넘버:
UserInfoNameRequired=이름을 입력해야 합니다.
WizardSelectDir=설치할 위치 선택
SelectDirDesc=[name] 을(를) 설치할 곳은 어디입니까?
SelectDirLabel3=설치 프로그램은 [name] 을(를) 다음 폴더에 설치할 것입니다.
SelectDirBrowseLabel=계속 하려면 "다음"을 클릭하세요. 다른 폴더를 선택하려면, "탐색"를 클릭하세요.
DiskSpaceMBLabel=최소 [mb] MB 의 디스크 여유 공간이 설치에 필요합니다.
ToUNCPathname=선택하신 네트워크 경로로 설치할 수 없습니다. 네트워크 드라이브에 설치하려면, 네트워크 드라이브에 접근할 수 있어야 합니다.
InvalidPath=드라이브 문자를 포함한 전체 경로를 입력해야 합니다. 예:%n%nC:\APP%n%n 네트워크 드라이브의 예:%n%n\\server\share
InvalidDrive=설치할 드라이브나 네트워크 경로가 존재하지 않거나 접근할 수 없습니다. 다른 경로를 선택하세요.
...
SelectComponentsLabel2=설치하고 싶은 구성요소는 선택하시고, 설치하고 싶지 않은 구성요소는 선택을 해제하세요. 설치를 계속할 준비가 되었다면 "다음"을 클릭하세요.
FullInstallation=전체 설치
CompactInstallation=최소 설치
CustomInstallation=사용자 설치
NoUninstallWarningTitle=구성요소 존재
NoUninstallWarning=설치 프로그램이 다음 구성요소가 이미 설치되어 있음을 발견했습니다.%n%n%1%n%n프로그램 제거시 이 구성요소들은 제거되지 않을 것입니다.%n%n그래도 계속 하시겠습니까?
ComponentSize1=%1 KB
ComponentSize2=%1 MB
ComponentsDiskSpaceMBLabel=선택한 구성요소 설치에 필요한 최소 용량: [mb] MB
WizardSelectTasks=추가 작업 선택
SelectTasksDesc=어떤 작업을 추가로 실행하시겠습니까?
SelectTasksLabel2=[name] 의 설치 과정에서 추가로 실행하고자 하는 사항을 선택하고, "다음"을 클릭하세요.
WizardSelectProgramGroup=시작 메뉴 폴더 선택
SelectStartMenuFolderDesc=프로그램 바로가기가 위치할 곳은 어디입니까?
SelectStartMenuFolderLabel3=다음의 시작 메뉴 폴더에 프로그램 바로가기를 만듭니다.
SelectStartMenuFolderBrowseLabel=계속 하려면 "다음"을 클릭하세요. 다른 폴더를 선택하려면, "탐색"를 클릭하세요.
...
NoProgramGroupCheck2=시작 메뉴 폴더를 만들지 않기
WizardReady=설치 준비 완료
ReadyLabel1=[name] 을(를) 설치할 준비가 되었습니다.
ReadyLabel2a="설치"를 클릭하여 설치를 시작하거나, "뒤로"를 클릭하여 설치 내용을 검토하거나 바꿀 수 있습니다.
ReadyLabel2b="설치"를 클릭하여 설치를 시작하세요.
ReadyMemoUserInfo=사용자 정보:
ReadyMemoDir=설치 경로:
ReadyMemoType=설치 형식:
ReadyMemoComponents=선택한 구성요소:
ReadyMemoGroup=시작 메뉴 폴더:
ReadyMemoTasks=추가적인 작업:
WizardPreparing=설치 준비 중...
PreparingDesc=설치 프로그램이 [name] 을(를) 설치할 준비를 하고 있습니다.
PreviousInstallNotCompleted=이전의 설치나 프로그램 제거 작업이 완료되지 않았습니다. 이전의 설치를 완료하기 위하여 컴퓨터를 다시 시작할 필요가 있습니다.%n%n컴퓨터를 다시 시작한 후, 설치 프로그램을 다시 시작하여 [name] 의 설치를 완료하세요.
CannotContinue=설치를 계속할 수 없습니다. "취소"를 클릭하여 설치를 종료하세요.
WizardInstalling=설치 중...
InstallingLabel=설치 프로그램이 [name] 을(를) 설치하는 동안 기다려 주세요.
FinishedHeadingLabel=[name] 설치 완료
FinishedLabelNoIcons=설치 프로그램이 [name] 의 설치를 완료했습니다.
FinishedLabel=설치 프로그램이 [name] 의 설치를 완료했습니다. 설치된 바로가기를 실행하시면 프로그램이 실행됩니다.
ClickFinish="완료"를 클릭하여 설치를 완료하세요.
...
ShowReadmeCheck=README 파일을 읽어 보겠습니다.
YesRadio=예, 지금 시스템을 다시 시작하겠습니다
NoRadio=아니오, 나중에 시스템을 다시 시작하겠습니다
RunEntryExec=%1 실행하기
RunEntryShellExec=%1 읽어보기
ChangeDiskTitle=다음 디스크가 필요
SelectDiskLabel2=디스크 %1 을(를) 넣고 "확인" 단추를 클릭하세요.%n%n만약 다음 디스크가 아래에 표시된 경로와 다른 폴더에 있다면 올바른 경로를 입력하거나 "탐색" 단추를 클릭해 올바른 경로를 지정하세요.
PathLabel=경로:
FileNotInDir2="%1" 파일이 "%2" 에 존재하지 않습니다. 올바른 디스크를 넣거나 다른 폴더를 선택하세요.
SelectDirectoryLabel=다음 디스크가 있는 경로를 지정하세요.
SetupAborted=설치가 완료되지 않았습니다.%n%n문제를 해결하고 설치 프로그램을 다시 시작하세요.
EntryAbortRetryIgnore=다시 시도하려면 "다시 시도"를, 무시하고 설치를 계속 하려면 "무시"를, 설치를 종료하려면 "취소"를 클릭하세요.
...
FileAbortRetryIgnore=다시 시도하려면 "다시 시도"를, 무시하려면 "무시"를 (권장하지 않음), 설치를 종료하려면 "취소"를 클릭하세요.
FileAbortRetryIgnore2=다시 시도하려면 "다시 시도"를, 무시하려면 "무시"를 (권장하지 않음), 설치를 종료하려면 "취소"를 클릭하세요.
SourceIsCorrupted=원본 파일이 손상되었습니다
SourceDoesntExist=원본 파일 "%1" 이(가) 존재하지 않습니다
ExistingFileReadOnly=다음 파일은 읽기 전용입니다.%n%n읽기 전용 속성을 제거하고 다시 시도하려면 "다시 시도"를, 이 파일을 무시하려면 "무시"를, 설치를 종료하려면 "취소"를 클릭하세요.
ErrorReadingExistingDest=파일을 읽는 도중 오류 발생:
FileExists=이미 존재하는 파일입니다.%n%n파일을 덮어쓰시겠습니까?
ExistingFileNewer=이미 존재하는 파일이 설치하려는 파일보다 새 버전입니다. 기존 파일을 보존할 것을 권장합니다.%n%n기존 파일을 보존하시겠습니까?
...
ErrorRestartingComputer=설치 프로그램이 시스템을 다시 시작할 수 없습니다. 수동으로 다시 시작하세요.
...
ConfirmUninstall=정말로 %1 와(과) 그 구성요소들을 완전히 제거하시겠습니까?
UninstallOnlyOnWin64=이 프로그램은 64비트 Windows 에서만 제거됩니다.
OnlyAdminCanUninstall=이 프로그램은 Administrator 권한이 있는 사용자만 제거할 수 있습니다.
UninstallStatusLabel=%1 이(가) 제거되는 동안 기다려 주세요.
UninstalledAll=%1 이(가) 완전히 제거되었습니다.
UninstalledMost=%1 의 제거가 끝났습니다.%n%n일부 항목은 제거할 수 없었습니다. 관련 항목들을 직접 제거하기 바랍니다.
...
ConfirmDeleteSharedFile2=시스템은 다음 공유 파일이 어떤 프로그램에서도 사용되지 않음을 발견했습니다. 다음 공유 파일을 삭제하시겠습니까?%n%n만약 이 공유 파일들이 다른 프로그램들에 의해서 사용된다면, 이 공유 파일 삭제 후 다른 프로그램들이 제대로 작동하지 않을 수 있습니다. 확실하지 않다면 "아니오" 를 클릭하세요. 파일을 남겨두어도 시스템에 영향을 끼치지 않습니다.
...
CreateDesktopIcon=바탕 화면에 아이콘 생성
CreateQuickLaunchIcon=빠른 실행에 아이콘 생성
ProgramOnTheWeb=웹상의 %1
UninstallProgram=%1 제거
LaunchProgram=%1 실행
AssocFileExtension=%2 파일 확장자를 %1 에 연결
AssocingFileExtension=%2 파일 확장자를 %1 에 연결 중...
