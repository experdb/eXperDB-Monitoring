...
boot()
{
      ...
      echo "   **********************************"
		...
		if [ "$PID" = "" ]; then
		    ...
		    java -Djava.rmi.server.hostname=${SERVER_HOSTNAME} -Dcom.sun.management.jmxremote.port=${JMXREMOTE_PORT} -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -D${DNAME} -Xmx2048m -DeXperDBMA.config=eXperDBMA.config -DeXperDBMA_HOME=${eXperDBMA_HOME}/ -DAGENT_HOME=${AGENT_HOME}/ -DCONFIG_DIR=config -classpath ../:../lib/commons-collections-3.1.jar:../lib/commons-dbcp-1.2.1.jar:../lib/commons-pool-1.2.jar:../lib/log4j-1.2.17.jar:../lib/mybatis-3.2.5.jar:../lib/postgresql-42.2.2.jre7.jar:../lib/commons-codec-1.9.jar:../lib/json-simple-1.1.1.jar:../lib/quartz-2.3.0-SNAPSHOT.jar:../lib/quartz-jobs-2.3.0-SNAPSHOT.jar:../lib/slf4j-log4j12-1.7.7.jar:../lib/slf4j-api-1.7.7.jar experdb.mnt.Server &
		    sleep 1
		    IS_eXperDBMA_PID
