...
public class RscCollect extends TaskApplication {
	...
	//					diskIoSelInput.put("collect_dt", preDiskIoTemp.get("collect_dt"));
			...
			try {
				...
				//금일자 최초 거래인지 확인
				HashMap<String, Object> regDateMap = sessionAgent.selectOne("app.TB_RSC_COLLECT_INFO_S001");
				...
				if(!regDateMap.get("max_reg_date").equals(regDateMap.get("reg_date")))
				{
					...
					sessionAgent.selectList("app.SEQ_SETVAL_RSC");
				}
				...
				for (HashMap<String, Object> map : cpuSel) {
					...
					HashMap<String, Object> nowMap = sessionAgent.selectOne("app.RSC_NOW");
					map.put("dt_now", nowMap.get("dt_now"));
